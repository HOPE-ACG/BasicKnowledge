...
public class LookupTest {
    ...
    public void testBinaryRecursion() {
        int[] nums = {5, 35, 1, 2, 56, 21, 14, 8};
        Quick quick = new Quick(nums);
        quick.algorithm(0, nums.length - 1);
        int[] numsSorted = quick.getNums();
        Binary binary = new Binary(numsSorted);
        System.out.println(binary.algorithm_recursion(6, 0, numsSorted.length - 1));
        System.out.println(binary.algorithm_recursion(35, 0, numsSorted.length - 1));
        System.out.println(binary.algorithm_recursion(1, 0, numsSorted.length - 1));
    }
}
